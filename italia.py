...
"""
todo Να βγάλω κωδικούς απο το αρχείο
todo Να στέλνει λίστα για τις τιμές απο τισ σελίδεσ
todo Να βάλω να μήν αλλάζει τα ID
todo Να φτιαξω τα κομματα με τελίες
todo Να το βάλω στο github
"""
import sys
...
from PyQt5.QtWidgets import QTableWidgetItem, QMessageBox, QProgressBar, QStatusBar, QHeaderView
from sql import get_spare_parts, save_prices, get_model, save_to_basket, get_basket, save_basket, get_prices_date
import ml_sql
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.konica_machine_label = QtWidgets.QLabel(self.konica_tab)
        self.konica_machine_label.setText("Μηχανήματα")
        self.konica_machine_label.setObjectName(u"konica_machine_label")
        self.konica_machine_label.setFont(font)
        self.konica_machine_label.setAlignment(Qt.AlignCenter)
        self.konica_machine_label.setStyleSheet("background-color: gray; color: white; max-height: 40px; "
                                                "border-top-right-radius: 20px; border-bottom-left-radius: 20px;")
        self.gridLayout.addWidget(self.konica_machine_label, 0, 0, 1, 1)
        ...
        self.konica_add_product_btn.setStyleSheet("background-color: rgb(85, 255, 255);\n"
                                                  "color: rgb(8, 8, 8);")
        ...
        self.tabWidget.addTab(self.konica_tab, "KONICA")
        self.sharp_tab = QtWidgets.QWidget()
        self.sharp_tab.setStatusTip("")
        self.sharp_tab.setWhatsThis("")
        self.sharp_tab.setAccessibleName("")
        self.sharp_tab.setAccessibleDescription("")
        self.sharp_tab.setObjectName("sharp_tab")
        self.gridLayout_3 = QtWidgets.QGridLayout(self.sharp_tab)
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.ar5516_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.ar5516_btn.setLocale(QtCore.QLocale(QtCore.QLocale.English, QtCore.QLocale.UnitedKingdom))
        self.ar5516_btn.setObjectName("ar5516_btn")
        self.ar5516_btn.clicked.connect(lambda: self.show_parts(tableWidget=self.sharp_tableWidget, machine_id=7))
        self.gridLayout_3.addWidget(self.ar5516_btn, 2, 0, 1, 1)
        self.sharp_italia_price_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.sharp_italia_price_btn.setMaximumSize(QtCore.QSize(16777215, 16777215))
        self.sharp_italia_price_btn.setStyleSheet("color: rgb(255, 255, 255); background-color: rgb(0, 0, 255);"
                                                  "")
        self.sharp_italia_price_btn.setObjectName("sharp_italia_price_btn")
        self.sharp_italia_price_btn.clicked.connect(lambda: self.update_ital_prices())
        self.gridLayout_3.addWidget(self.sharp_italia_price_btn, 0, 3, 1, 1)
        self.ar5623_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.ar5623_btn.setObjectName("ar5623_btn")
        self.ar5623_btn.clicked.connect(lambda: self.show_parts(tableWidget=self.sharp_tableWidget, machine_id=8))
        self.gridLayout_3.addWidget(self.ar5623_btn, 3, 0, 1, 1)
        self.ar6023_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.ar6023_btn.setObjectName("ar6023_btn")
        self.ar6023_btn.clicked.connect(lambda: self.show_parts(tableWidget=self.sharp_tableWidget, machine_id=9))
        self.gridLayout_3.addWidget(self.ar6023_btn, 4, 0, 1, 1)

        self.sharp_machine_label = QtWidgets.QLabel(self.sharp_tab)
        self.sharp_machine_label.setText("Μηχανήματα")
        self.sharp_machine_label.setObjectName(u"sharp_machine_label")
        self.sharp_machine_label.setFont(font)
        self.sharp_machine_label.setAlignment(Qt.AlignCenter)
        self.sharp_machine_label.setStyleSheet("background-color: gray; color: white; max-height: 40px; min-height: "
                                               "40px; border-top-right-radius: 20px; border-bottom-left-radius: 20px;")
        self.gridLayout_3.addWidget(self.sharp_machine_label, 0, 0, 1, 1)
        self.ar5320_btn = QtWidgets.QPushButton(self.sharp_tab)
        # self.ar5320_btn.setMaximumSize(QtCore.QSize(100, 16777215))
        self.ar5320_btn.setObjectName("ar5320_btn")
        self.ar5320_btn.clicked.connect(lambda: self.show_parts(tableWidget=self.sharp_tableWidget, machine_id=6))
        self.gridLayout_3.addWidget(self.ar5320_btn, 1, 0, 1, 1)
        self.sharp_info_price_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.sharp_info_price_btn.setMaximumSize(QtCore.QSize(16777215, 16777215))
        self.sharp_info_price_btn.setStyleSheet("background-color: rgb(170, 255, 0); color: rgb(15, 15, 15);")
        self.sharp_info_price_btn.setObjectName("sharp_info_price_btn")
        self.sharp_info_price_btn.clicked.connect(lambda: self.update_info_prices())
        self.gridLayout_3.addWidget(self.sharp_info_price_btn, 0, 4, 1, 1)

        self.sharp_update_total_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.sharp_update_total_btn.setText("Υπολογισμός \n Συνόλου")
        self.sharp_update_total_btn.clicked.connect(lambda: self.calc_total())
        self.gridLayout_3.addWidget(self.sharp_update_total_btn, 0, 5, 1, 1)

        # Sharp Model Label
        self.sharp_model_label = QtWidgets.QLabel(self.sharp_tab)
        self.sharp_model_label.setMaximumSize(QtCore.QSize(350, 16777215))
        self.sharp_model_label.setFont(font)
        self.sharp_model_label.setAlignment(Qt.AlignCenter)
        self.sharp_model_label.setStyleSheet("background-color: gray; max-height: 40px; border-top-right-radius: 20px; \
                                border-bottom-left-radius: 20px; background-color: #ff9800; max-width: 200px ;min-width: 200px")

        self.gridLayout_3.addWidget(self.sharp_model_label, 0, 2, 1, 1)
        self.sharp_add_product_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.sharp_add_product_btn.setStyleSheet("color: rgb(0, 0, 0);\n"
                                                 "background-color: rgb(85, 255, 255);")
        self.sharp_add_product_btn.setObjectName("sharp_add_product_btn")
        self.gridLayout_3.addWidget(self.sharp_add_product_btn, 10, 1, 1, 1)
        self.sharp_sent_to_basket_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.sharp_sent_to_basket_btn.setStyleSheet("background-color: rgb(0, 170, 0);\n"
                                                    "color: rgb(255, 255, 255);")
        self.sharp_sent_to_basket_btn.setObjectName("sharp_sent_to_basket_btn")
        self.sharp_sent_to_basket_btn.clicked.connect(lambda: self.send_to_basket())
        self.gridLayout_3.addWidget(self.sharp_sent_to_basket_btn, 10, 5, 1, 1)
        self.sharp_tableWidget = QtWidgets.QTableWidget(self.sharp_tab)
        # Sharp Table Widget
        self.sharp_tableWidget.setToolTip("")
        self.sharp_tableWidget.setStatusTip("")
        self.sharp_tableWidget.setWhatsThis("")
        self.sharp_tableWidget.setAccessibleName("")
        self.sharp_tableWidget.setAccessibleDescription("")
        self.sharp_tableWidget.setAutoFillBackground(True)
        self.sharp_tableWidget.setLocale(QtCore.QLocale(QtCore.QLocale.Greek, QtCore.QLocale.Greece))
        self.sharp_tableWidget.setAlternatingRowColors(True)
        self.sharp_tableWidget.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        self.sharp_tableWidget.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
        self.sharp_tableWidget.setObjectName("sharp_tableWidget")
        self.sharp_tableWidget.setColumnCount(11)
        self.sharp_tableWidget.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.sharp_tableWidget.resizeRowsToContents()
        self.sharp_tableWidget.resizeColumnsToContents()
        ...
        item.setText("ID")
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(0, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Part Nr.")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(1, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Κωδικός")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(2, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Τεμ.")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(3, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Ital-Κωδικός")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(4, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Ital-Τιμή")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(5, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Info-Κωδικός")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(6, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Info-Τιμή")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(7, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Info-site-κωδικός")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(8, item)

        item = QtWidgets.QTableWidgetItem()
        item.setText("Τεμ.")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        item.setFont(font)
        self.sharp_tableWidget.setHorizontalHeaderItem(9, item)
        ...
        self.sharp_tableWidget.setHorizontalHeaderItem(10, item)
        # self.sharp_tableWidget.horizontalHeader().setDefaultSectionSize(110)

        self.gridLayout_3.addWidget(self.sharp_tableWidget, 1, 1, 9, 5)
        self.sharp_search_edit = QtWidgets.QLineEdit(self.sharp_tab)
        self.sharp_search_edit.setMaximumSize(QtCore.QSize(280, 16777215))
        self.sharp_search_edit.setObjectName("sharp_search_edit")
        self.sharp_search_edit.setPlaceholderText("Αναζήτηση...")
        self.sharp_search_edit.textChanged.connect(lambda: self.search(self.sharp_search_edit))
        self.gridLayout_3.addWidget(self.sharp_search_edit, 0, 1, 1, 1)
        self.xm264_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.xm264_btn.setObjectName("xm264_btn")
        self.xm264_btn.clicked.connect(lambda: self.show_parts(tableWidget=self.sharp_tableWidget, machine_id=10))
        self.gridLayout_3.addWidget(self.xm264_btn, 5, 0, 1, 1)
        self.mxm266_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.mxm266_btn.setObjectName("mxm266_btn")

        self.mxm266_btn.clicked.connect(lambda: self.show_parts(tableWidget=self.sharp_tableWidget, machine_id=11))
        self.gridLayout_3.addWidget(self.mxm266_btn, 6, 0, 1, 1)
        self.sharp_save_changes_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.sharp_save_changes_btn.setStyleSheet("background-color: rgb(85, 0, 127);")
        self.sharp_save_changes_btn.setObjectName("sharp_save_changes_btn")
        self.sharp_save_changes_btn.clicked.connect(lambda: self.save_changes(tableWidget=self.sharp_tableWidget))
        self.gridLayout_3.addWidget(self.sharp_save_changes_btn, 10, 3, 1, 1)
        self.sharp_add_model_btn = QtWidgets.QPushButton(self.sharp_tab)
        self.sharp_add_model_btn.setObjectName("sharp_add_model_btn")
        self.gridLayout_3.addWidget(self.sharp_add_model_btn, 10, 0, 1, 1)
        self.tabWidget.addTab(self.sharp_tab, "")
        ...
        self.history_date_select_combobox.addItem("")
        ...
        sizePolicy.setHeightForWidth(self.search_history_edit.sizePolicy().hasHeightForWidth())
        self.search_history_edit.setSizePolicy(sizePolicy)
        self.search_history_edit.setMaximumSize(QtCore.QSize(200, 16777215))
        self.search_history_edit.setObjectName("search_history_edit")
        self.gridLayout_5.addWidget(self.search_history_edit, 2, 1, 1, 1)
        self.search_history_btn = QtWidgets.QPushButton(self.history_tab)
        # self.search_history_btn.setMaximumSize(QtCore.QSize(100, 16777215))
        self.search_history_btn.setObjectName("search_history_btn")
        self.gridLayout_5.addWidget(self.search_history_btn, 2, 2, 1, 1)
        self.history_tableWidget = QtWidgets.QTableWidget(self.history_tab)
        self.history_tableWidget.setMinimumSize(QtCore.QSize(700, 0))
        ...
        item = QtWidgets.QTableWidgetItem()
        item.setText("Drum")
        self.history_tableWidget.setVerticalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.history_tableWidget.setVerticalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.history_tableWidget.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        item.setText("Ημερομηνία")
        item.setTextAlignment(QtCore.Qt.AlignCenter)
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        ...
        item.setForeground(brush)
        item.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsUserCheckable | QtCore.Qt.ItemIsEnabled)
        self.history_tableWidget.setItem(0, 0, item)
        item = QtWidgets.QTableWidgetItem()
        item.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsUserCheckable | QtCore.Qt.ItemIsEnabled)
        self.history_tableWidget.setItem(0, 1, item)
        item = QtWidgets.QTableWidgetItem()
        item.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsUserCheckable | QtCore.Qt.ItemIsEnabled)
        self.history_tableWidget.setItem(0, 2, item)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.bzh215_btn.setText(_translate("MainWindow", "215"))
        # Konica Table Widget
        ...
        self.bzh361_btn.setText(_translate("MainWindow", "361"))

        self.bzh210_btn.setText(_translate("MainWindow", "210"))
        ...
        self.bzh36_btn.setText(_translate("MainWindow", "36"))
        self.bzh282_btn.setText(_translate("MainWindow", "282"))
        self.konica_add_model_btn.setText(_translate("MainWindow", "Προσθήκη"))
        self.ar5516_btn.setText(_translate("MainWindow", "AR-5516"))
        self.sharp_italia_price_btn.setText(_translate("MainWindow", "Εύρεση τιμών Ιταλία"))
        self.ar5623_btn.setText(_translate("MainWindow", "AR-5623"))
        self.ar6023_btn.setText(_translate("MainWindow", "AR-6023"))
        self.ar5320_btn.setText(_translate("MainWindow", "AR-5320"))
        self.sharp_info_price_btn.setText(_translate("MainWindow", "Εύρεση τιμών Info-copy"))

        self.sharp_add_product_btn.setText(_translate("MainWindow", "Προσθήκη Προϊόντος"))
        self.sharp_sent_to_basket_btn.setText(_translate("MainWindow", "Προσθήκη στο καλάθι"))

        self.xm264_btn.setText(_translate("MainWindow", "MX-M264"))
        self.mxm266_btn.setText(_translate("MainWindow", "MX-M266"))
        self.sharp_save_changes_btn.setText(_translate("MainWindow", "Αποθήκευση Αλλαγών"))
        self.sharp_add_model_btn.setText(_translate("MainWindow", "Προσθήκη"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.sharp_tab), _translate("MainWindow", "SHARP"))
        ...
        self.history_date_select_combobox.setItemText(0, _translate("MainWindow", "Ημερομηνία"))
        self.search_history_btn.setText(_translate("MainWindow", "Αναζήτηση"))
        # History
        self.history_tableWidget.setSortingEnabled(True)
        item = self.history_tableWidget.verticalHeaderItem(1)
        item.setText(_translate("MainWindow", "Τελικό Σύνολο"))
        ...
        item.setText(_translate("MainWindow", "Τιμή"))
        item = self.history_tableWidget.horizontalHeaderItem(6)
        item.setText(_translate("MainWindow", "Σύνολο"))
        __sortingEnabled = self.history_tableWidget.isSortingEnabled()
        self.history_tableWidget.setSortingEnabled(False)
        self.history_tableWidget.setSortingEnabled(__sortingEnabled)
    ...
    def show_parts(self, tableWidget=None, machine_id=None):

        try:
            self.selected_tableWidget = tableWidget
            self.selected_machine_id = machine_id
            data_to_show = get_spare_parts(machine_id)
            machine_model = get_model(machine_id)
            if "Konica" in machine_model:
                self.konica_model_label.setText(machine_model)
            elif "Sharp" in machine_model:
                self.sharp_model_label.setText(machine_model)
            ...
            for row, data in enumerate(data_to_show):
                ...
                if "Konica" in machine_model:
                    pieces = ml_sql.get_ml_konica_pieces(ml_code.text())
                else:
                    pieces = ml_sql.get_ml_sharp_pieces(ml_code.text())
                ...
                # if data.info_site_code != "0":
                #     # info_price = asyncio.run(ital_scrap.main(info_site_code=data.info_site_code))
                #     info_price = get_info_price(data.info_site_code)
                # else:
                #     info_price = 0
            ...
            # Αποκόμηση ημερωμηνίας τελευταίας ενημέρωσης τιμών
    ...
    def update_ital_prices(self):
        try:
            ...
            rows = self.selected_tableWidget.rowCount()
            # ProgressBar
            self.progressBar.setStyleSheet(u"color: rgb(0, 0, 255);")
            ...
            for row in range(0, rows):
                ...
                ital_price = QTableWidgetItem(str(ital_price))
                self.selected_tableWidget.setItem(row, 5, ital_price)
            msgBox = QMessageBox.information(None, "Πληροφορία", "Η διαδικασία ηταν επιτυχής")
        except Exception:
            msgBox = QMessageBox.critical(None, "Σφάλμα", f"{traceback.print_exc()}")
            print(traceback.print_exc())
        finally:
    ...
    def search(self, search_edit):
        ...
        self.selected_tableWidget.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
